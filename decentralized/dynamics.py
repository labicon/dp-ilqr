...
class DynamicalModel(abc.ABC):
    ...
    def plot(self, X, do_headings=False, coupling_radius=None):
        ...
        assert (
            X.shape[1] >= 3
        ), "Must at least have x and y states for this to make sense."
        ...
        if do_headings:
            ...
            ends = (
                bases
                + dists[:, np.newaxis] * np.vstack([np.cos(theta), np.sin(theta)]).T
            )
            ...
            for i in range(N - 1):
                plt.annotate(
                    ...
                    ends[i],
                    bases[i],
                    arrowprops=dict(facecolor="black", headwidth=5, width=1, shrink=0),
                )
...
class NumericalDiffModel(DynamicalModel):
    ...
    def linearize(self, x, u):
        A = np.vstack(
            [
                approx_fprime(x, lambda x: self.f(x, u)[i], self.j_eps)
        ...
        B = np.vstack(
            [
                approx_fprime(u, lambda u: self.f(x, u)[i], self.j_eps)
                for i in range(self.n_x)
            ]
        )
