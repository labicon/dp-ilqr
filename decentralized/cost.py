...
class ObstacleCost(Cost):
    ...
    def __call__(self, x, _, __=None):
        distance = np.sqrt(
            (x[self.ix] - self.point.x) ** 2 + (x[self.iy] - self.point.y) ** 2
        )
    ...
    def quadraticize(self, x, u, _=None):
        ...
        L_x_obs, L_xx_obs = _quadraticize_distance(
            self.state_point(x), self.point, self.radius
        )
    ...
    def quadraticize(self, x, _=None, __=None):
        ...
        for i in range(self.n_agents):
            for j in range(i + 1, self.n_agents):
                ...
                L_xxi[ix, iy] = L_xxi[iy, ix] = L_xxi[jx, jy] = L_xxi[
                    jy, jx
                ] = L_xx_pair[IX, IY]
...
class AgentCost(Cost):
    ...
    def __init__(self, costs):
        assert all(
            isinstance(cost, ReferenceCost) or isinstance(cost, ObstacleCost)
            for cost in costs
        )
        self.costs = costs
...
class MultiAgentCost(Cost):
    ...
    def __init__(self, agent_costs, coupling_costs, x_dims, u_dims):
    ...
    def __call__(self, x, u, terminal=False):
        ...
        agent_total = sum(
            agent_cost(xi, ui, terminal)
            for agent_cost, xi, ui in zip(self.agent_costs, x_split, u_split)
        )
        coupling_total = sum(
            coupling_cost.weight * coupling_cost(x)
    ...
    def plot(
        self, surface_plot=False, agent_ind=0, couple_ind=0, axis=None, log_colors=False
    ):
        ...
        pts = np.mgrid[axis[0] : axis[1] : STEP, axis[2] : axis[3] : STEP].T.reshape(
            -1, 2
        )
...
def _quadraticize_distance(point_a, point_b, max_distance):
    ...
    L_xx[IX, IX] = (
        2 * max_distance * dx ** 2 / distance ** 3 - 2 * max_distance / distance + 2
    )
    ...
    L_xx[IY, IY] = (
        2 * max_distance * dy ** 2 / distance ** 3 - 2 * max_distance / distance + 2
    )
