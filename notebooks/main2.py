...
def proximity_cost(x, _x_dims, _radius):
    ...
    distances = torch.linalg.norm(
        x_agent[:2, pair_inds[:, 0]] - x_agent[:2, pair_inds[:, 1]], dim=0
    )
...
def multi_agent_sim():
    ...
    x_dims = [4, 4]
    u_dims = [2, 2]
    x = torch.tensor([-5, -5, 0, 0, -5, 5, 0, 0], dtype=torch.float, requires_grad=True)
    x_goal = torch.tensor([[5, 5, 0, 0, 5, -4, 0, 0]], dtype=torch.float).T

    dynamics_8d = functools.partial(
        dynamics_nd, unicycle_continuous, _x_dims=x_dims, _u_dims=u_dims
    )

    Q = torch.diag(torch.tensor([1.0, 1, 0, 0]).tile(2))
    Qf = 1000 * torch.eye(Q.shape[0])
    R = torch.eye(4)
    radius = 0.5
